...
subroutine mini(xopt,obs,weight,n,nwl,nspec, fun)
...
allocate (x(n), prf(nwl), ipiv(n), work(45*n), sigc(n,n), diag(n), rhsold(n), deriva(nwl,n), derivc(n,n), rhs(n))
...
!  for weights and normalization of misfit function.
 ...
 call fun(x,prf,deriva,n,'grad',nspec)
! exit !!!
...
!  Evaluate the function only
  ...
  call fun(x,prf,deriva,n,'func',nspec)
  call deviat(obs,prf,weight,nwl,d,dev)
